...
const fiatCurrency = computed(() => props.tradeInfo.offerResponse.offer.fiat_currency)
const fiatPriceByRate = computed(() => {
  return calculateFiatPriceByRate(priceClient.getPrice(fiatCurrency.value), props.tradeInfo.offerResponse.offer.rate)
})
...
const tradeType = computed(() => {
  return props.tradeInfo.offerResponse.offer.offer_type === 'buy' ? 'Buy' : 'Sell'
})
