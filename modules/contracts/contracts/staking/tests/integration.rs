...
fn cw20receive_enter_and_leave() {
    ...
    // Check if Alice's LOCAL balance is 100
    ...
    // We can unstake LOCAL only by calling the xLOCAL token.
    ...
    // Tru to stake Alice's 100 LOCAL for 100 xLOCAL
    let msg = Cw20ExecuteMsg::Send {
        contract: staking_instance.to_string(),
    ...
    // Check if Alice's xLOCAL balance is 100
    let msg = Cw20QueryMsg::Balance {
        address: alice_address.to_string(),
    };
    let res: Result<BalanceResponse, _> =
    ...
    // Check if Alice's LOCAL balance is 0
    let msg = Cw20QueryMsg::Balance {
        address: alice_address.to_string(),
    };
    let res: Result<BalanceResponse, _> =
    ...
    // Check if the staking contract's LOCAL balance is 100
    let msg = Cw20QueryMsg::Balance {
    ...
    // We can stake tokens only by calling the LOCAL token.
    let msg = Cw20ExecuteMsg::Send {
        contract: staking_instance.to_string(),
        msg: to_binary(&Cw20HookMsg::Enter {}).unwrap(),
        amount: Uint128::from(10u128),
    ...
    // Try to unstake Alice's 10 xLOCAL for 10 LOCAL
    let msg = Cw20ExecuteMsg::Send {
        contract: staking_instance.to_string(),
        msg: to_binary(&Cw20HookMsg::Leave {}).unwrap(),
        amount: Uint128::from(10u128),
    ...
    // Check if Alice's xLOCAL balance is 90
    let msg = Cw20QueryMsg::Balance {
        address: alice_address.to_string(),
    };
    let res: Result<BalanceResponse, _> =
    ...
    // Check if Alice's LOCAL balance is 10
    let msg = Cw20QueryMsg::Balance {
        address: alice_address.to_string(),
    };
    let res: Result<BalanceResponse, _> =
    ...
    // Check if the staking contract's LOCAL balance is 90
    let msg = Cw20QueryMsg::Balance {
        address: staking_instance.to_string(),
    };
    let res: Result<BalanceResponse, _> =
    ...
    // Check if the staking contract's xLOCAL balance is 0
    let msg = Cw20QueryMsg::Balance {
        address: staking_instance.to_string(),
    };
    let res: Result<BalanceResponse, _> =
...
fn should_not_allow_withdraw_more_than_what_you_have() {
    ...
    // enter Alice's 100 LOCAL for 100 xLOCAL
    let msg = Cw20ExecuteMsg::Send {
        contract: staking_instance.to_string(),
        msg: to_binary(&Cw20HookMsg::Enter {}).unwrap(),
        amount: Uint128::from(100u128),
    ...
    // Try to burn Alice's 200 xLOCAL and unstake
    let msg = Cw20ExecuteMsg::Send {
        contract: staking_instance.to_string(),
        msg: to_binary(&Cw20HookMsg::Leave {}).unwrap(),
...
fn should_work_with_more_than_one_participant() {
    ...
    // Stake Alice's 20 LOCAL for 20 xLOCAL
    let msg = Cw20ExecuteMsg::Send {
        contract: staking_instance.to_string(),
        msg: to_binary(&Cw20HookMsg::Enter {}).unwrap(),
    ...
    // Stake Bob's 10 LOCAL for 10 xLOCAL
    let msg = Cw20ExecuteMsg::Send {
        contract: staking_instance.to_string(),
        msg: to_binary(&Cw20HookMsg::Enter {}).unwrap(),
        amount: Uint128::from(10u128),
    ...
    // Check if Alice's xLOCAL balance is 20
    let msg = Cw20QueryMsg::Balance {
        address: alice_address.to_string(),
    };
    let res: Result<BalanceResponse, _> =
    ...
    // Check if Bob's xLOCAL balance is 10
    let msg = Cw20QueryMsg::Balance {
    ...
    // Check if staking contract's LOCAL balance is 30
    let msg = Cw20QueryMsg::Balance {
        address: staking_instance.to_string(),
    };
    let res: Result<BalanceResponse, _> =
    ...
    // Staking contract gets 20 more LOCAL from external source
    ...
    // Stake Alice's 10 LOCAL for 6 xLOCAL: 10*30/50 = 6
    let msg = Cw20ExecuteMsg::Send {
        contract: staking_instance.to_string(),
        msg: to_binary(&Cw20HookMsg::Enter {}).unwrap(),
        amount: Uint128::from(10u128),
    ...
    // Check if Alice's xLOCAL balance is 26
    let msg = Cw20QueryMsg::Balance {
        address: alice_address.to_string(),
    };
    let res: Result<BalanceResponse, _> =
    ...
    // Burn Bob's 5 xLOCAL and unstake: gets 5*60/36 = 8 LOCAL
    let msg = Cw20ExecuteMsg::Send {
        contract: staking_instance.to_string(),
        msg: to_binary(&Cw20HookMsg::Leave {}).unwrap(),
    ...
    // Check if Bob's xLOCAL balance is 5
    let msg = Cw20QueryMsg::Balance {
        address: bob_address.to_string(),
    };
    let res: Result<BalanceResponse, _> =
    ...
    // Check if the staking contract's LOCAL balance is 52 (60 - 8 (Bob left 5 xLOCAL))
    let msg = Cw20QueryMsg::Balance {
        address: staking_instance.to_string(),
    };
    let res: Result<BalanceResponse, _> =
    ...
    // Check if Alice's LOCAL balance is 70 (100 minted - 20 entered - 10 entered)
    let msg = Cw20QueryMsg::Balance {
        address: alice_address.to_string(),
    };
    let res: Result<BalanceResponse, _> =
    ...
    // Check if Bob's LOCAL balance is 98 (100 minted - 10 entered + 8 by leaving)
    let msg = Cw20QueryMsg::Balance {
        address: bob_address.to_string(),
    };
    let res: Result<BalanceResponse, _> =
